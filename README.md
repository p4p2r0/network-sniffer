...
2. Each captured packet is walked layer by layer using Scapy's dissection —
   its actual class chain (`Ether`/`IP`/`TCP`/...) — producing a summary row
   (time, source, destination, protocol, length) and a full field tree.
...
## Usage
Everything is driven from the interface — there are no CLI flags. Pick an
interface, optionally set a BPF filter (e.g. `tcp port 443`), toggle PCAP
if you want a `.pcap` saved, then Start. `q` quits, `c` clears the table.
